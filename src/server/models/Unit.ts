...
export default class Unit {
    ...
    public get target(): Unit {
        return this.world.units.getUnit(this.data.target);
    }
    ...
    private takeHit(dmg: number, attacker: Unit): void {
        ...
        if (this.dead) {
            ...
            // TODO: drop tables
            this.world.groundItems.addGroundItem({
                item: {
                    uuid: uuid(),
                    itemid: 0,
                    icon: 81,
    ...
    public attackUnit(target: Unit): void {
        this.state = UnitState.ATTACKING;
        this.target = target;
    }

    public followUnit(target: Unit): void {
